...
public class PDUUtil {
	...
	public PDUUtil(String no, boolean notype, String msg) {
		number = no;
		message = msg;
		numbertype = notype;
	}

	public static String getMessageInPDUFormat() {
		return getMessageInPDUFormat(message);
	}

	public static String getMessageInPDUFormat(String message) {
		byte[] msg = compress(convertUnicode2GSM(message));
		return new String(byteArrayToHexString(msg));
	}
	...
	public static String getSMSText(String data) {
		...
		x = Integer.parseInt(s, 10); // get length [byte] of delivering SMSC
										// number
		i = 2 + x * 2; // set index to message header flags
		i = i + 2; // set index to length [digits] of originating adress
		s = data.substring(i, i + 2); // get length [digits] of originating
										// adress
		...
		// get data and time
		date = data.substring(i + 1, i + 2) + data.substring(i, i + 1); // get
																		// year
		date = data.substring(i + 3, i + 4) + data.substring(i + 2, i + 3) + "." + date; // get
																							// month
		date = data.substring(i + 5, i + 6) + data.substring(i + 4, i + 5) + "." + date; // get
																							// day

		time = data.substring(i + 11, i + 12) + data.substring(i + 10, i + 11); // get
																				// hour
		time = data.substring(i + 9, i + 10) + data.substring(i + 8, i + 9) + ":" + time; // get
																							// minute
		time = data.substring(i + 7, i + 8) + data.substring(i + 6, i + 7) + ":" + time; // get
																							// second

		i = i + 14; // set index to length of user data (=SMS)
		s = data.substring(i, i + 2); // get length of user data (=SMS)
		x = Integer.parseInt(s, 16); // calculate length [characters] of user
										// data (=SMS)
		data = data.substring(i + 2, data.length()); // delete the transport
														// information at the
	 ...
	 */
	public static char[] byteArrayToHexString(byte[] data) {
		int l = data.length;
